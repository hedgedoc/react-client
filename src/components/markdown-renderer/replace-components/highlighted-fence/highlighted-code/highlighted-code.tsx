...
export const HighlightedCode: React.FC<HighlightedCodeProps> = ({ code, language, startLineNumber, wrapLines }) => {
  ...
  useEffect(() => {
    import('highlight.js').then((hljs) => {
      ...
      const unreplacedCode = replacedLanguage && languageSupported(replacedLanguage) ? hljs.highlight(replacedLanguage, code).value : escapeHtml(code)
      ...
      setDom(replacedDom)
    }).catch(() => { console.log('error while loading highlight.js') })
