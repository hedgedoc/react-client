...
describe('find regex index in line', function () {
  it('finds the first occurrence', () => {
    const result = findRegexMatchInLine('aba', /a/g, 0)
  ...
  it('finds another occurrence', () => {
    const result = findRegexMatchInLine('aba', /a/g, 1)
    expect(result).toBeDefined()
    expect(result).toHaveLength(1)
  ...
  it('fails to find with a wrong regex', () => {
    const result = findRegexMatchInLine('aba', /c/g, 0)
  ...
  it('fails to find if the index is to high', () => {
    const result = findRegexMatchInLine('aba', /a/g, 100)
    expect(result).not.toBeDefined()
  })
})
